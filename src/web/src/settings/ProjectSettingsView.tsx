...
export function ProjectSettingsView() {
  ...
  return <><PageHeader title={`${key} settings`} /><SettingsLayout>
    ...
    {me.administrator && <Section title="Project lifecycle" description="Deletion is reversible during the instance grace period."><Button variant="destructive" onClick={() => { if (window.confirm(`Delete project ${key}?`)) void api.DELETE("/projects/{key}", { params: { path: { key: key! } } }).then(r => { if (r.response.ok) window.location.assign("/"); else setNotice(describe(r.error, r.response.status)); }); }}>Delete project</Button></Section>}
