...
export function AdminView() {
  ...
  async function invite(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const form = e.currentTarget; const d = new FormData(form); const r = await api.POST("/users", { body: { name: String(d.get("name")), email: String(d.get("email")), administrator: d.has("administrator") } }); setNotice(r.data ? "Invitation sent." : "The invitation could not be sent."); if (r.data) { form.reset(); await load(); } }
  ...
  return <><PageHeader title="Instance administration" /><SettingsLayout>
    <Section title="Users" description="Invite users and manage their instance role and lifecycle."><form className="mb-3 grid gap-2 sm:grid-cols-[1fr_1fr_auto]" onSubmit={(e) => void invite(e)}><Input name="name" placeholder="Name" aria-label="Name" /><Input name="email" type="email" placeholder="Email" aria-label="Email" /><Button type="submit">Invite</Button><label className="flex gap-2 text-sm sm:col-span-3"><input name="administrator" type="checkbox" /> Administrator</label></form><div className="divide-y rounded-md border">{users.map(u => <Row key={u.id} title={u.name} detail={`${u.email} · ${u.state}${u.administrator ? " · administrator" : ""}`} action={<div className="flex gap-1">{u.state === "invited" && <Button size="sm" variant="outline" onClick={() => void api.POST("/users/{id}/invitation", { params: { path: { id: u.id } } }).then(() => setNotice("Invitation resent."))}>Resend</Button>}<Button size="sm" variant="outline" onClick={() => void api.PATCH("/users/{id}", { params: { path: { id: u.id } }, body: { administrator: !u.administrator } }).then(load)}>{u.administrator ? "Demote" : "Make admin"}</Button><Button size="sm" variant="outline" onClick={() => void api.POST(u.state === "deactivated" ? "/users/{id}/reactivate" : "/users/{id}/deactivate", { params: { path: { id: u.id } } }).then(load)}>{u.state === "deactivated" ? "Reactivate" : "Deactivate"}</Button></div>} />)}</div></Section>
    <Section title="Projects" description="All projects, including deleted ones, and their project access."><div className="divide-y rounded-md border">{projects.map(p => <div key={p.key} className="p-3"><Row title={`${p.key} · ${p.name}`} detail={p.deleted_at ? `Deleted ${new Date(p.deleted_at).toLocaleString()}` : `Access: ${(access[p.key] ?? []).map(x => x.name).join(", ") || "nobody"}`} action={p.deleted_at && <Button size="sm" variant="outline" onClick={() => void api.POST("/projects/{key}/restore", { params: { path: { key: p.key } } }).then(load)}>Restore</Button>} />{!p.deleted_at && <form className="mt-2 flex gap-2" onSubmit={(e) => { e.preventDefault(); const id = String(new FormData(e.currentTarget).get("user")); if (grantable(p.key).length === 0) return; void api.PUT("/projects/{key}/users/{id}", { params: { path: { key: p.key, id } } }).then(r => { if (!r.response.ok) setNotice(describe(r.error, r.response.status)); return load(); }); }}><select name="user" aria-label={`User for ${p.key}`} className="h-8 flex-1 rounded-md border bg-background px-2 text-sm">{grantable(p.key).map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select><Button type="submit" size="sm" variant="outline" disabled={grantable(p.key).length === 0}>Grant access</Button>{(access[p.key] ?? []).map(u => <Button key={u.id} type="button" size="sm" variant="ghost" onClick={() => void api.DELETE("/projects/{key}/users/{id}", { params: { path: { key: p.key, id: u.id } } }).then(load)}>Remove {u.name}</Button>)}</form>}</div>)}</div></Section>
    <Section title="Transactional email" description="Credentials remain in environment variables.">{smtp && <p className="mb-3 text-sm">{smtp.configured ? `${smtp.host}:${smtp.port} · ${smtp.security} · ${smtp.sender}` : "Not configured"}</p>}<form className="flex max-w-lg gap-2" onSubmit={(e) => { e.preventDefault(); const email = String(new FormData(e.currentTarget).get("email")); void api.POST("/admin/smtp/test", { body: { email } }).then(r => setNotice(r.response.ok ? "Test email sent." : "Test email failed.")); }}><Input name="email" type="email" placeholder="Test recipient" aria-label="Test recipient" /><Button type="submit" disabled={!smtp?.configured}>Send test</Button></form></Section>
