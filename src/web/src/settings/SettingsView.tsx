...
export function SettingsView() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>, action: (data: FormData) => Promise<void>) { event.preventDefault(); setNotice(""); try { await action(new FormData(event.currentTarget)); setNotice("Saved."); } catch (error) { setNotice(error instanceof Error ? error.message : "The instance did not answer."); } }
  ...
  return <><PageHeader title="Personal settings" /><SettingsLayout>
    <Section title="Profile" description="Your name is used on comments and history.">
      <form className="flex max-w-lg gap-2" onSubmit={(e) => void submit(e, async data => { const r = await api.PATCH("/me", { body: { name: String(data.get("name")) } }); if (!r.data) throw new Error(describe(r.error, r.response.status)); window.location.reload(); })}><Input name="name" defaultValue={me.name} aria-label="Name" /><Button>Save name</Button></form>
      <form className="mt-3 flex max-w-lg gap-2" onSubmit={(e) => void submit(e, async data => { const r = await api.POST("/me/email", { body: { email: String(data.get("email")) } }); if (!r.response.ok) throw new Error(describe(r.error, r.response.status)); setNotice("Check the new address to confirm the change."); })}><Input name="email" type="email" defaultValue={me.email ?? ""} aria-label="Email" /><Button variant="outline">Change email</Button></form>
    </Section>
    <Section title="Password" description="Changing it signs every other browser session out.">
      <form className="grid max-w-lg gap-2" onSubmit={(e) => void submit(e, async data => { const r = await api.POST("/me/password", { body: { current_password: String(data.get("current")), password: String(data.get("password")) } }); if (!r.response.ok) throw new Error(describe(r.error, r.response.status)); e.currentTarget.reset(); })}><Input name="current" type="password" placeholder="Current password" aria-label="Current password" /><Input name="password" type="password" placeholder="New password (12 characters or more)" minLength={12} aria-label="New password" /><Button className="w-fit">Change password</Button></form>
    </Section>
    ...
    <Section title="Agents" description="Each agent has one token and inherits your project access."><form className="mb-3 flex max-w-lg gap-2" onSubmit={(e) => void submit(e, async data => { const r = await api.POST("/agents", { body: { name: String(data.get("name")) || null } }); if (!r.data) throw new Error(); setSecret(r.data.token.secret); await load(); e.currentTarget.reset(); })}><Input name="name" placeholder="Optional agent name" aria-label="Agent name" /><Button>Create agent</Button></form><Rows empty="No agents.">{agents.map(a => <Row key={a.id} title={a.name} detail={`${a.token.prefix}… · ${a.token.revoked_at ? "revoked" : "active"}`} action={<div className="flex gap-1"><Button variant="outline" size="sm" onClick={() => { const name = window.prompt("Agent name", a.name); if (name) void api.PATCH("/agents/{id}", { params: { path: { id: a.id } }, body: { name } }).then(load); }}>Rename</Button>{!a.token.revoked_at && <Button variant="outline" size="sm" onClick={() => void api.DELETE("/agents/{id}", { params: { path: { id: a.id } } }).then(load)}>Revoke</Button>}</div>} />)}</Rows></Section>
