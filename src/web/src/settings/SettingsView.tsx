...
export function SettingsView() {
  ...
  return <><PageHeader title="Personal settings" /><SettingsLayout>
    ...
    <Section title="Password" description="Changing it signs every other browser session out.">
      <form className="grid max-w-lg gap-2" onSubmit={(e) => void submit(e, async data => { const r = await api.POST("/me/password", { body: { current_password: String(data.get("current")), password: String(data.get("password")) } }); if (!r.response.ok) throw new Error(describe(r.error, r.response.status)); e.currentTarget.reset(); })}><Input name="current" type="password" placeholder="Current password" aria-label="Current password" /><Input name="password" type="password" placeholder="New password (12 characters or more)" minLength={12} aria-label="New password" /><Button type="submit" className="w-fit">Change password</Button></form>
    </Section>
