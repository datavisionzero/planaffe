...
export function IssueView() {
  ...
  const changed = (value: Issue) => { setState((old) => ({ key: key!, issue: { at: "known", value }, history: old?.history ?? asking })); setEditing(false); };
  ...
  return <><PageHeader title={<span className="flex items-center gap-2"><span className="font-mono text-xs font-normal text-muted-foreground">{issue.key}</span>{issue.title}</span>}><Button variant="outline" size="sm" onClick={() => setEditing(true)}>Edit</Button></PageHeader>
    <div className="flex flex-1 flex-col md:flex-row">
      <main className="min-w-0 flex-1 p-4 md:p-6">
        <Attention issue={issue} project={project!} onChanged={changed} />
...
function Attention({ issue, project, onChanged }: { issue: Issue; project: string; onChanged: (issue: Issue) => void }) {
