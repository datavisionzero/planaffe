...
/// <summary>
/// The short code <c>pa login</c> prints and a human types into a browser on
/// some other machine (ADR 0025). Eight characters, shown as <c>XXXX-XXXX</c>.
/// </summary>
/// <remarks>
/// <para>
...
/// is the failure mode of every short random code a person is asked to read
/// aloud. No digits means the confusions that make people retype — <c>0</c> and
...
/// </para>
/// <para>
/// Eight characters is 20^8, about 2.6 × 10^10, and the code lives ten minutes.
/// That is not a token's kind of unguessable and does not have to be: what it
/// protects is a request that still needs a signed-in human to press a button.
/// The credential is <see cref="DeviceCode"/> and carries 256 bits.
/// </para>
/// </remarks>
public static class UserCode
{
    ...
    /// all forgiven — they are how the code was shown, not what it is. Empty
    /// when what arrived is not a code at all, so that a lookup for nothing
    /// finds nothing rather than throwing.
    /// </summary>
    ...
    public static string ForReading(string code) =>
        code?.Length == Length ? $"{code[..GroupLength]}-{code[GroupLength..]}" : code ?? string.Empty;
}
