...
// newPage is what a person does with a page of the knowledge base. It stood
...
// ambiguity this product must not have. The wiki is withdrawn (VISION 18), the
// word is free, and a page is simply a page again.
//
// Nothing else moved with it. The address is the same one argument — the space
// in front, the slugs from the root down behind it — and every verb is the
// verb it already was.
func newPage(g *globals) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "page",
		Short: "Pages of the knowledge base: Markdown addressed by the space and the slugs from it down.",
	}
	cmd.AddCommand(newPageList(g), newPageView(g), newPageCreate(g), newPageEdit(g), newPageRename(g), newPageMove(g), newPageDelete(g), newPageRestore(g))
	return cmd
}
...
// newPageList is the tree, slim and without the bodies — the sibling of
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(*resp.JSON200) == 0 {
				fmt.Fprintf(cmd.OutOrStdout(), "No pages in %s yet; `pa page create %s/<slug> --title …` writes the first.\n", args[0], args[0])
				return nil
			}
...
// newPageCreate takes the address the page is to have, and reads the tree
...
// newPageEdit changes the text and nothing about where the page stands.
...
func newPageEdit(g *globals) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(changes) == 0 {
				return &config.UsageError{Message: "nothing to change: --title or --body-file. The slug is `pa page rename`, the parent `pa page move`."}
			}
			return changePage(g, cmd, at, changes, ifMatch)
		},
	}
...
// newPageRename is its own verb rather than a flag on `edit`, because
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.Contains(args[1], "/") {
				return &config.UsageError{Message: "a rename takes a slug, not an address: where the page hangs is `pa page move`."}
			}
			return changePage(g, cmd, at, map[string]any{"slug": args[1]}, ifMatch)
		},
	}
	cmd.Flags().StringVar(&ifMatch, "if-match", "", "the updated_at as last read; refused as stale when it moved")
	return cmd
...
func changePage(g *globals, cmd *cobra.Command, at address, changes map[string]any, ifMatch string) error {
	_, c, err := g.load()
	if err != nil {
		return err
	}
...
// newPageMove hangs the page under another parent, with everything below
...
func newPageMove(g *globals) *cobra.Command {
	var under string
	cmd := &cobra.Command{
...
// newPageDelete takes the subtree with it and says how many went. A
...
func deleted(at string, many int) string {
	if many > 1 {
		return fmt.Sprintf("%s deleted with the %d pages under it; `pa page restore %s` brings them back.", at, many-1, at)
	}
	return fmt.Sprintf("%s deleted; `pa page restore %s` brings it back.", at, at)
}

// newPageRestore brings back exactly what went along. A page whose parent
