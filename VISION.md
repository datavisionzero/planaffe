...
**The switch: review required.** Whether an agent's close lands in `review` or in `done` is a project switch, off by default. It sits opposite triage required (10.) — triage guards the entrance, review guards the exit — but only the exit still asks whose word it is: triage decides what `next` hands out, and who writes `ready` is a convention rather than a rule (10.). Off, an agent's close is a close, and `done` means what the agent says it means; whoever trusts their agents clicks nothing. On, every close by an agent lands in `review` — `canceled` included, because "could not be done" is a claim to check as much as "done" is, and the `result` carries the reason — and `done` is a human's word, which is what makes the release a record rather than a claim (7.). A human closing a ticket goes straight to `done` either way; the switch is about what an agent's word is worth, which is a question worth asking at the exit, where the work is already done and nobody else will look.
...
- **On**: only flagged tickets are pulled. Nothing without `ready` is handed out, so a ticket nobody has looked at sits still — useful when several people create tickets, or when you want to look over them yourself after a breakdown, before five agents set off.

**The switch governs the selection, not the writing.** Whoever may change the ticket may write `ready`, whatever kind of token they hold. That `ready` is a human's word is a **convention**, worded like the one about answering questions above: a human says it, and an agent says it when told to. It was a server rule once — an agent could clear the flag and never set it — and it contradicted the paragraph before it: triage happens in the chat, and "tidy these up and release the three that are clear" is exactly the sentence that rule refused. It also ran along the wrong line. What it forbade was the *token kind*, so the same person doing the same thing under their user token was allowed; it protected less than it cost ([ADR 0019](docs/adr/0019-triage-required-selects-it-does-not-permit.md)). The history keeps saying who set the flag, which is where that question is answered.

Review required (9.) is not pulled along with it: what an agent's word is worth at the **exit** is a different question from who may say a ticket is concrete at the entrance, and the answer there stands.
...
- **"Needs you"** — what only a human can resolve, and nothing else: open questions first, then tickets in `review`, then — only where triage is required — tickets without `ready`, and blocked tickets only when the chain of blockers ends in something no agent can pull: a parked ticket, or one with an open question. A ticket blocked by a ticket an agent will do needs nobody and is noise here; it waits, visibly, in "in progress" — "waiting for PLAN-40". A parked ticket is not on the list itself: parking is the decision that it is not up yet, and there is nothing there to resolve. That there is no agent to pull anything at all is a fact about the installation and not about a ticket, so the answer says it once beside the list — the same thought as "an empty result explains itself", which is what makes the one act it calls for visible instead of scattering it over every blocked ticket. This is the human's work list: provide supply so the agents do not run dry.
