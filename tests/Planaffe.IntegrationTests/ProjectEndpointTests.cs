...
public sealed class ProjectEndpointTests(PostgresFixture postgres)
{
    ...
    /// <summary>
    /// The text every agent is handed with every ticket (VISION 15.3): a user
    /// writes it, an agent may not, and it arrives inside the context package
    /// rather than on a route of its own. It is a field on the project since
    /// VISION 18 — the one text that cannot live in a space, because it has to
    /// follow project access and no second rule.
    /// </summary>
    [Fact]
    public async Task The_instructions_are_written_by_a_user_and_travel_with_every_ticket()
    {
        await using var instance = await AnInstance.BootstrappedAsync(postgres);
        using var admin = instance.ClientWith(AnInstance.BootstrapToken);
        await admin.PostAsJsonAsync("/projects", new { key = "PLAN", name = "planaffe" }, Ct);
        ...
        // Written by nobody yet: the package carries the ticket and no instructions.
        var project = await admin.GetFromJsonAsync<JsonElement>("/projects/PLAN", Ct);
        Assert.Equal(JsonValueKind.Null, project.GetProperty("instructions").ValueKind);
        ...
        using var written = await admin.PatchAsJsonAsync("/projects/PLAN", new { instructions = "Tests run with `just test`." }, Ct);
        Assert.Equal(HttpStatusCode.OK, written.StatusCode);
        Assert.Equal(
            "Tests run with `just test`.",
            (await written.Content.ReadFromJsonAsync<JsonElement>(Ct)).GetProperty("instructions").GetString());

        // In the package, as the text itself — and in the same object every act
        // answers with, which is what makes claiming deliver it.
        var read = await admin.GetFromJsonAsync<JsonElement>("/issues/PLAN-1", Ct);
        Assert.Equal("Tests run with `just test`.", read.GetProperty("project_context").GetProperty("instructions").GetString());
        ...
        Assert.Equal(
            ...
            (await claimed.Content.ReadFromJsonAsync<JsonElement>(Ct))
                .GetProperty("project_context").GetProperty("instructions").GetString());

        // An agent may not write them: it would be writing its own brief.
        using var byAgent = await agent.PatchAsJsonAsync("/projects/PLAN", new { instructions = "Ask nobody." }, Ct);
        await Problem(byAgent, HttpStatusCode.Forbidden, "forbidden");

        // Leaving the field out leaves it alone; `null` takes it away, and so
        // does a blank text — a project carrying an empty instruction carries none.
        using var kept = await admin.PatchAsJsonAsync("/projects/PLAN", new { name = "planaffe" }, Ct);
        Assert.Equal("Tests run with `just test`.", (await kept.Content.ReadFromJsonAsync<JsonElement>(Ct)).GetProperty("instructions").GetString());

        using var blanked = await admin.PatchAsJsonAsync("/projects/PLAN", new { instructions = "   " }, Ct);
        Assert.Equal(JsonValueKind.Null, (await blanked.Content.ReadFromJsonAsync<JsonElement>(Ct)).GetProperty("instructions").ValueKind);

        using var rewritten = await admin.PatchAsJsonAsync("/projects/PLAN", new { instructions = "No dependency lands without asking." }, Ct);
        Assert.Equal(HttpStatusCode.OK, rewritten.StatusCode);

        using var cleared = await admin.PatchAsJsonAsync("/projects/PLAN", new { instructions = (string?)null }, Ct);
        Assert.Equal(JsonValueKind.Null, (await cleared.Content.ReadFromJsonAsync<JsonElement>(Ct)).GetProperty("instructions").ValueKind);
        Assert.Equal(
            JsonValueKind.Null,
