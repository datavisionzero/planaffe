...
pa issue list --status todo --label bug --assignee me --sort priority   # a row is key, priority, status, the epic where the page has any, title
...
pa agent rotate AGENT                      # the next token, once; the one it held stops working
