...
create unique index token_agent on token (identity_id) where kind = 'agent' and revoked_at is null;
```
...
**An agent has exactly one token that works**, and the partial unique index
holds it — it counts only the rows that are not revoked. Creating an agent
creates identity and token in one transaction; revoking the token revokes the
agent; rotating it revokes that row and adds the next in one transaction, so an
agent never holds two that work and keeps every one it ever held
([ADR 0026](./adr/0026-an-agents-token-is-rotated-and-revoking-is-no-longer-a-dead-end.md)).
Nothing else gives an agent a second token. A user has as many as they create.
...
identity everywhere it ever acted; it simply fails authentication — and for an
agent it steps out of the index, leaving room for the one that replaces it.
...
**`instructions` is the text every agent is handed with its ticket** (VISION
15.3, `CONTEXT.md`, Instructions). One text per project and not a flag on any
number of documents, so the context an agent carries cannot grow by marking
more of them. It is a column and not a reference to a document elsewhere,
because it travels inside the context package and so has to follow project
access and no second rule (VISION 18): in a space it could be closed to the
agent that needs it. `null` and not the empty string where a project carries
none — the write path collapses a blank text into `null`, so the two never
disagree.
